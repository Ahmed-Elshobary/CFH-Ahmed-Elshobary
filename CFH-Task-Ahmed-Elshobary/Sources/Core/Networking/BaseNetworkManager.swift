...
//        paramaters.merge(staticParamters) { (_, new) in new }
    ...
    func setupAuth() {
      //  headers.append(.init(name: "AUTH", value: "Bearer " + (UserDefaultsManager.shared.getUser()?.auth_token ?? "")))
        print(headers)
    }
                            ...
                            Authentication.shared.refreshToken { [weak self] success in
                                if success {
                                    ...
                                    if var mutableRequest = try? request.convertible.urlRequest?.asURLRequest() {
                                        ...
                                        self?.beginRequest(for: updatedRequest, model: model, withSave: save)
                                    } else {
